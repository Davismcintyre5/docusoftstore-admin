...
const PaymentVerificationList = ({ payments, onRefresh }) => {
  ...
  // Use the screenshot URL directly – already a full GitHub URL
  const getImageUrl = (screenshotUrl) => {
    if (!screenshotUrl) return null;
    if (screenshotUrl.startsWith('http://') || screenshotUrl.startsWith('https://')) {
      return screenshotUrl;
    }
    return `https://docusoftserver.pxxl.click${screenshotUrl.startsWith('/') ? screenshotUrl : `/${screenshotUrl}`}`;
  };
              ...
              {payments.map(payment => {
                ...
                return (
                  <tr key={payment._id}>
                    ...
                    <td style={{ padding: '12px 16px', borderBottom: '1px solid #e2e8f0', fontWeight: '600' }}>{formatKES(payment.amount)}</td>
                    <td style={{ padding: '12px 16px', borderBottom: '1px solid #e2e8f0', fontSize: '13px' }}>{formatDate(payment.createdAt)}</td>
                    <td style={{ padding: '12px 16px', borderBottom: '1px solid #e2e8f0' }}>
                      {payment.screenshotUrl ? (
                        <button onClick={() => setViewImage(imageUrl)} style={{ padding: '6px 12px', fontSize: '12px', backgroundColor: '#4299e1', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
                          📸 View
                        </button>
                      ) : (
                    ...
                    <td style={{ padding: '12px 16px', borderBottom: '1px solid #e2e8f0' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button className="btn btn-success" style={{ padding: '6px 12px', backgroundColor: '#48bb78' }} onClick={() => handleApprove(payment._id)} disabled={processingId === payment._id}>
                          {processingId === payment._id ? '...' : '✓ Approve'}
                        </button>
                        <button className="btn btn-danger" style={{ padding: '6px 12px', backgroundColor: '#f56565' }} onClick={() => handleReject(payment._id)} disabled={processingId === payment._id}>
                          ✗ Reject
                        </button>
                      </div>
                    </td>
